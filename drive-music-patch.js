...
(() => {
  ...
  style.textContent=`
    ...
    @media(max-width:820px){
      ...
      #driveMusicBox .field input{font-size:16px!important;min-height:46px}
      #driveMusicBox .btn{min-height:44px;padding:10px 13px}
      #driveMusicBox>div{display:grid!important;grid-template-columns:1fr 1fr!important}
      #driveMusicBox #driveApply{grid-column:1/-1}
  ...
  function improveLogin(){
    const email=document.getElementById('authEmail'),pw=document.getElementById('authPassword');
    if(!email||!pw)return;
    email.setAttribute('autocomplete','username');email.setAttribute('autocapitalize','none');email.setAttribute('inputmode','email');
    ...
    const original=window.loginCloud;
    window.loginCloud=async()=>{if(document.getElementById('rememberEmail')?.checked)localStorage.setItem(EMAIL_KEY,email.value.trim());else localStorage.removeItem(EMAIL_KEY);return original?.()};
    pw.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();window.loginCloud?.()}});
  }
  function enhance(){ensureDriveBox();simplifyAndMove();improveLogin()}
